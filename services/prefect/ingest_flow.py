...
def upsert_csv(table: str, csv_path: str, pk_cols: list[str]):
    ...
    # Conversion de certains types si nécessaire
    if "signup_date" in df.columns:
        df["signup_date"] = pd.to_datetime(df["signup_date"], errors="coerce")

    # Conversion en booléen pour les colonnes pertinentes
    for col in ["plan_stream_tv", "plan_stream_movies", "paperless_billing",
                "add_on_security", "add_on_backup", "add_on_device_protect", "add_on_support"]:
    ...
    with eng.begin() as conn:
        ...
        # Construire la partie SET pour les colonnes non PK
