...
def main():
    ...
    with mlflow.start_run(run_name=f"rf_baseline_{AS_OF}") as run:
        ...
        mlflow.sklearn.log_model(
            sk_model=pipe,          # on log la pipeline complète
            artifact_path="model",
            registered_model_name=MODEL_NAME,
            signature=signature
        )

        print(f"Nombre de lignes du dataset d'entraînement : {df.shape[0]}")
        print("Colonnes catégorielles détectées (cat_cols) :", cat_cols)
